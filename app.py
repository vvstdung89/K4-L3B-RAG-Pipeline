...
from src.task10_generation import LLM_MODEL, LLM_PROVIDER, generate_with_citation, reorder_for_llm  # noqa: E402
...
def render_sources(sources: list[dict], retrieval_source: str) -> None:
    ...
    with st.expander("Nguồn tham khảo", expanded=False):
        for index, source in enumerate(reorder_for_llm(sources), 1):
            metadata = source["metadata"]
            location = metadata.get("url") or metadata.get("source", "")
            st.markdown(
                f"**[Document {index}] {metadata.get('title', source['id'])}** "
...
st.caption("Luật Du lịch, visa nhập cảnh và cẩm nang điểm đến — câu trả lời kèm citation [Document N] khớp với danh sách nguồn.")
