  ...
  if (!summary) {
    return (
      <Card className="p-space-xl">
        <Empty icon="science">
          Chưa có kết quả đánh giá. Chạy <code className="font-mono">python -m src.task11_evaluation</code>.
        </Empty>
      </Card>
    );
  }
    ...
    <div className="flex flex-col gap-space-lg pb-space-xl">
      ...
      <Card className="overflow-hidden">
        <div className="p-space-lg border-b border-slate-100">
          <CardTitle icon="compare_arrows" title={`So sánh cấu hình A/B (${summary.method ?? 'RAGAS'})`} subtitle={summary.artifact ?? 'group_project/evaluation/results/summary.json'} />
        </div>
        <table className="w-full text-left font-body-md text-body-md">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            <tr>
              ...
              {names.map((name) => (
                <td key={name} className="px-space-md py-space-sm font-mono">{summary.configs[name].source_hit_rate == null ? '—' : `${(summary.configs[name].source_hit_rate! * 100).toFixed(1)}%`}</td>
              ))}
            ...
            <div key={index}>
              ...
              >
                <span className="truncate">
                  ...
                  {row.hit_expected_source === false && <span className="text-error font-semibold">[miss nguồn] </span>}
                  {row.question}
                </span>
                {METRICS.map((m) => (
